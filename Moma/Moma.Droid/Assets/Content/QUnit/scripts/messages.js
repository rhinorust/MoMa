...
$('document').ready(function () {
    ...
    var bleTitle = "";
    if (currentLanguage == "en") bleTitle = "Messages";
    if (currentLanguage == "fr") bleTitle = "Message";
    messageBoxTitle.text(bleTitle);

    var pointsOfInterestBla = "";
    if (currentLanguage == "en") pointsOfInterestBla = "Points of interest";
    if (currentLanguage == "fr") pointsOfInterestBla = "Point d'intérêt";
    messageBox.find("#content #iBeaconsBla").text(pointsOfInterestBla);

    var noPointsFoundYet = "";
    if (currentLanguage == "en") noPointsFoundYet = "<p id='delMe'>No points of interest have been discovered yet.</p>";
    if (currentLanguage == "fr") noPointsFoundYet = "<p id='delMe'>Aucun point d'intérêt découvert.</p>";
    ...
    if (typeof storylineSelectedID === 'undefined') {
        var qrCodesBla = "";
        if (currentLanguage == "en") qrCodesBla = "QRCodes";
        if (currentLanguage == "fr") qrCodesBla = "Code QR";
        messageBox.find("#content #qrCodesBla").text(qrCodesBla);

        var noQRFoundYet = "";
        if (currentLanguage == "en") noQRFoundYet = "<p id='delMe'>No QRCodes have been scanned yet.</p>";
        if (currentLanguage == "fr") noQRFoundYet = "<p id='delMe'>Aucun code QR découvert.</p>";
